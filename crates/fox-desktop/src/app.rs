...
use crate::components::{loading_overlay::LoadingOverlay, project_tree::SideBar, toasts::Toasts, topbar::TopBar};
    ...
    let _ = desktop.create_wry_event_handler(move |event, _| {
        if let dioxus::desktop::tao::event::Event::WindowEvent { event: we, .. } = event {
            if matches!(we, dioxus::desktop::WindowEvent::CloseRequested) {
                let state = crate::window_state::WindowState::from_window(&desktop_for_handler.window);
